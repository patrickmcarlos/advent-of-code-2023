...
const processData = (input: string) => {
    ...
    rows.forEach((row) => {
        ...
        const numMatches = findNumMatches(winningNumbers, myNumbers);
...
const findNumMatches = (
    ...
    for (const myNumber of myNumbers) {
        winningNumbers.has(myNumber) && numMatches++;
    }

    return numMatches;
};

const findTotalNumOfCards = (cardData: CardData[]) => {
    const cardCollection = new Array(cardData.length).fill(1);

    for (let i = 0; i < cardCollection.length; i++) {
        const { numMatches } = cardData[i];

        for (
            let j = i + 1;
            j < Math.min(i + 1 + numMatches, cardCollection.length);
            j++
        ) {
            cardCollection[j] += cardCollection[i];
        }
    }

    return cardCollection.reduce((acc, curr) => acc + curr);
};

console.log(findTotalNumOfCards(processData(data)));
